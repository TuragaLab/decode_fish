...
__all__ = ['eval_logger', 'load_from_eval_dict', 'train']
...
def train(model,
    ...
    for batch_idx in range(num_iter_sl + num_iter_ae):
        ...
        # Get supervised loss
        sim_vars = PointProcessUniform(local_rate, min_int=min_int).sample()  # sim_vars = locs_sl, x_os_sl, y_os_sl, z_os_sl, ints_sl, output_shape
        xsim = microscope(*sim_vars)
        xsim_noise = microscope.noise(xsim, background).sample()

        out_sim = model(xsim_noise)
        gmm_loss = -PointProcessGaussian(**out_sim).log_prob(*sim_vars[:-1]).mean()
        ...
        if batch_idx > num_iter_sl:
            ...
            ae_img = microscope(*model_output_to_micro_input(out_inp, threshold=0.1))
        ...
        if batch_idx % log_interval == 0:
            ...
            if writer is not None:
                with torch.no_grad():
                    pred_df = model_output_to_df(out_sim, 0.1)
                    target_df = sample_to_df(*sim_vars[:-1])
                    eval_logger(writer, pred_df, target_df, batch_idx, data_str='Sim. ')

                    if log_figs:
                        sl_fig = sl_plot(x, xsim_noise, pred_df, background, out_sim)
                        plt.show()
...
#                             writer.add_figure('GT', eval_fig, batch_idx)
            ...
            #storing
            torch.save({'state_dict':model.state_dict(), 'scaling':[model.unet.inp_scale, model.unet.inp_offset]}, str(save_dir) +'/model.pkl')
            torch.save(microscope.state_dict(), str(save_dir) + '/microscope.pkl')
            torch.save(psf.state_dict(), str(save_dir) + '/psf.pkl' )
            torch.save(optim_psf.state_dict(), str(save_dir) + '/opt_psf.pkl' )
            torch.save(optim_net.state_dict(), str(save_dir) + '/opt_net.pkl' )