...
__all__ = ['eval_logger', 'save_train_state', 'train']
...
#             torch.save({'state_dict':model.state_dict(), 'scaling':[model.inp_scale, model.inp_offset]}, save_dir/f'model_{batch_idx}.pkl')
    ...
    #                 print(len(pred_df), len(target_df))
                    ...
                    if cfg.evaluation.code_stats.enabled:
                        ...
                        hydra.utils.call(cfg.evaluation.code_stats.eval_func, model=model, bench_df=bench_df, post_proc=post_proc,
                                         targets=targets, crop=crop, image_vol=dl.dataset.volumes, wandb=wandb, batch_idx=batch_idx, chrom_map=get_color_shift_inp(microscope.color_shifts, microscope.col_shifts_yx)[:,:,None],
                                         scale = microscope.get_ch_mult().detach())
