...
#         return psf_deformed[:,None]
    ...
    def apply_color_shifts(self, col_inds, locations, x_os_val, y_os_val):
        ...
        if len(ch_inds[1]):
            if ch_inds[1].max() > 0:
                ...
                if self.col_shifts_enabled and ycrop is not None:
                    ...
                    col_inds = torch.tensor(self.ch_cols)[ch_inds[1]]
                    x_os_val, y_os_val = self.apply_color_shifts(col_inds, locations, x_os_val, y_os_val)
...
def add_pos_noise(offset_vars, scales, n_bits):
    """
    ...
    Currently doesn't support codebooks that use varying numbers of bits.
    """
