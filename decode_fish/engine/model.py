...
__all__ = ['number_of_features_per_level', 'create_conv', 'SingleConv', 'DoubleConv', 'Upsampling', 'Encoder',
           'Decoder', 'Abstract3DUNet', 'UNet3D', 'IntensityDist', 'init_weights', 'OutputNet', 'UnetDecodeNoBn',
           'UnetDecodeNoBn_2S']
...
class UnetDecodeNoBn_2S(nn.Module):
    ...
    """
    def __init__(self, ch_in=1, depth=3, inp_scale=1., inp_offset=0., order='ce', f_maps=64,
                 is_2D=False, pred_z=True, p_offset=-5., int_conc=4., int_rate=1., int_loc=1., n_p_ch=1, n_bg_ch=1, n_int_ch=1, n_out_l=1, chrom_map=False, resid_x=False, two_stage=True):
        super().__init__()
        ...
        if not two_stage:
            ...
            ch_in = self.n_ch + 2 if self.chrom_map else self.n_ch
            self.unet = UNet3D(ch_in, final_sigmoid=False, num_levels=depth, is_2D=is_2D, layer_order = order, f_maps=f_maps)

            self.network = nn.ModuleList([self.unet])

        else:

            self.single_n_ch = 12
            ch_in = self.n_ch * self.single_n_ch
            if chrom_map: ch_in = ch_in + 2

            self.single_ch_unet = UNet3D(1, final_sigmoid=False, num_levels=1, is_2D=is_2D, layer_order = order, f_maps=self.single_n_ch)
            self.unet = UNet3D(ch_in, final_sigmoid=False, num_levels=depth, is_2D=is_2D, layer_order = order, f_maps=f_maps)
            self.network = nn.ModuleList([self.single_ch_unet, self.unet])

        self.outnet = OutputNet(f_maps=f_maps, p_offset=p_offset, is_2D=is_2D, n_p_ch=n_p_ch, n_bg_ch=n_bg_ch, n_int_ch=n_int_ch, n_out_l=n_out_l, resid_x_ch=resid_x*self.n_ch)
        self.network.append(self.outnet)
        self.int_dist = IntensityDist(int_conc, int_rate, int_loc)
    ...
    def forward(self, x):
        ...
        if self.two_stage:
            s_x = x[:,:-2] if self.chrom_map else x
            single_ch_out = self.network[0](s_x.flatten(0,1)[:,None]).reshape(s_x.shape[0],-1,*s_x.shape[2:])
            m_x = torch.cat([single_ch_out, x[:,-2:]], 1) if self.chrom_map else single_ch_out
            unet_out = self.network[1](m_x)
        else:
            unet_out = self.network[0](x)

        if self.resid_x:
            res_x = x[:,:-2] if self.chrom_map else x
